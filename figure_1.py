...
def transition(iR,iP,Zr,Zp,r,h,mu,beta,Mcb):
...
def transition_matrix(iV,Zr,Zp,r,h,mu,beta,Mcb):
    ...
    for idx, i in enumerate(iV):
        ...
        if iR < Zr:
            W[i2idx[(iR + 1, iP)], idx] = transition(iR,iP,Zr,Zp,r,h,mu,beta,Mcb)[0]

        if iR > 0:
            W[i2idx[(iR - 1, iP)], idx] = transition(iR,iP,Zr,Zp,r,h,mu,beta,Mcb)[1]

        if iP < Zp:
            W[i2idx[(iR, iP + 1)], idx] =  transition(iR,iP,Zr,Zp,r,h,mu,beta,Mcb)[2]

        if iP > 0:
            W[i2idx[(iR, iP - 1)], idx] = transition(iR,iP,Zr,Zp,r,h,mu,beta,Mcb)[3]

        # probability that the population state does not transition from (iR, iP)
        W[(idx, idx)] = 1 - sum(transition(iR,iP,Zr,Zp,r,h,mu,beta,Mcb))

    return W


def gradient(iV,r,h,mu,beta,Mcb):
    # transition probability matrix between population states (iR, iP) -> (iR', iP')
    ...
    for idx, i in enumerate(iV):
        ...
        TiR_gain = transition(iR,iP,Zr,Zp,r,h,mu,beta,Mcb)[0]
        TiR_loss = transition(iR,iP,Zr,Zp,r,h,mu,beta,Mcb)[1]
        TiP_gain = transition(iR,iP,Zr,Zp,r,h,mu,beta,Mcb)[2]
        TiP_loss = transition(iR,iP,Zr,Zp,r,h,mu,beta,Mcb)[3]
...
def graph(suffix, Zr, Zp, iV,P,r,h,mu,beta,Mcb, nG):
    ...
    im = ax.imshow(P, origin='lower', cmap='Greys', alpha=0.5)
    # fig.colorbar(im)
    ax.quiver(iRV, iPV, gradient(iV,r,h,mu,beta,Mcb)[0], gradient(iV,r,h,mu,beta,Mcb)[1])
...
def axes(Zr,Zp,h,mu,beta):
    ...
    nG_list = []
    for r in risk_list:
        W = transition_matrix(combination, Zr, Zp,r,h,mu,beta,Mcb)
        pi = p(W)
...
Z = 40 #Number of individuals in the population
Zr = 8 #Number of rich individuals in the population
...
#plot_fig1('test_fig_1',mu,beta)

######################### Main code #########################
print(fDP(10,4,r,Mcb))
print(fCP(10,4,r,Mcb))
print(fCR(10,4,r,Mcb))
print(fDR(10,4,r,Mcb))

'''iV = iV(Zr,Zp)
W = transition_matrix(iV,Zr,Zp,r,h,mu,beta,Mcb)
p = p(W)
nG = nG(p,iV,Mcb)
P = P(p,iV)
graph('_figure_1A',Zr,Zp,iV,P,r,h,mu,beta,Mcb,nG)'''
